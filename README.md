...
- As of 8:27AM PST on 6/7/20, multiple endpoints on the JSONPlaceHolder API do not work, even when tested on Postman (status code 520). This has been a recurring issue. After clicking on a user, if the page is stuck on 'Loading Photos...', please go back a page in the browser, refresh the home page, and click on another User (it seems like User 2, Ernie Howell, works pretty consistently).
  - After some manual testing, it seems that the PhotoList component can break/take an extremely long time to render if a single API call fails. Thus, it is assumed that API calls will be successfully made, or else rendering will take an egregious amount of time.
